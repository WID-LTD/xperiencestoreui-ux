...
window.editAdminProduct = async (productId, currentCategory, currentSponsored) => {
    ...
    const sponsored = await Components.ConfirmAsync('Confirm Action', 'Mark as Sponsored (Featured on Homepage)?');
    if (category !== null) {
...
window.completeReceipt = async (orderId) => {
    if (!(await Components.ConfirmAsync('Confirm Action', 'Are you sure you want to mark this receipt as completed? This will update inventory levels.'))) return;
...
window.submitWRO = async (orderId) => {
    if (!(await Components.ConfirmAsync('Confirm Action', 'Submit Warehouse Receiving Order (WRO) with commercial invoice and packing list?'))) return;
...
window.completeWROReceipt = async (poId) => {
    if (!(await Components.ConfirmAsync('Confirm Action', 'Complete receipt for this WRO? This will update warehouse inventory and process supplier payout.'))) return;
...
window.editAdminUser = async (userId, currentRole, isVerified) => {
    ...
    const verifiedStatus = await Components.ConfirmAsync('Confirm Action', 'Is user verified?');
...
window.editAdminProduct = async (productId) => {
    ...
    const isSponsored = await Components.ConfirmAsync('Confirm Action', `Make "${product.name}" a sponsored listing?`);
...
window.deleteAdminProduct = async (productId) => {
    if (!(await Components.ConfirmAsync('Confirm Action', 'Are you sure you want to delete this product? This action cannot be undone.'))) return;
...
window.bulkAdminVerify = async () => {
    ...
    if (!(await Components.ConfirmAsync('Confirm Action', `Are you sure you want to verify ${selected.length} users?`))) return;
